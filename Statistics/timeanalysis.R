...
time_dat<- subset(koco_r2_complete, select = c("ind_id","R2_Result","date_R2"))
...
ggplot(prev, aes(x=week_cat,y=prev_est))+
  ...
  geom_errorbar(aes(ymin = prev_2.5, ymax =prev_97.5.5, colour=cat), position = position_dodge(width = 0.9),
                width = 0.2)+
  ylab("Prevalence")+xlab("Recruitment Round 2")+
  labs(colour="Method")+theme(legend.position="bottom")+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))

prev$outcome<-"DBS + Blood Plasma"

prev_dbs<- read.csv("prev_DBS_week.csv",header = TRUE)
prev_dbs$X<-NULL
prev<- rbind(prev, prev_dbs)
ggplot(prev, aes(x=week_cat,y=prev_est))+
  geom_point(aes(colour=cat),size=1.25,position = position_dodge(width = 0.9))+
  geom_errorbar(aes(ymin = prev_2.5, ymax =prev_97.5.5, colour=cat), position = position_dodge(width = 0.9),
                width = 0.2)+
  ylab("Prevalence")+xlab("Recruitment Round 2")+
  labs(colour="Method")+ facet_grid(.~outcome)+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+ theme_bw()+theme(legend.position="bottom")

ggsave("DBS vs DBS+Blood Plasma.jpeg", height=7, width=7,units="in",dpi=300)
